...
def update_product(product_id):
    ...
    if request.method == "POST":
        ...
        )  # Default to current category if not provided
    ...
    return render_template(
        "update_product.html", product=product, categories=categories
    )
...
def delete_product(product_id):
    ...
    return render_template(
        "confirm_delete.html", product=product, cart_items=cart_items
    )
        ...
        if cart:
            ...
            if cart_item:
                ...
                if cart_item.quantity > 1:
                    ...
                    db.session.commit()
                    flash(
                        f"One item of {product_name} removed from cart.",
                        "success",
                    )
                else:
                    db.session.delete(cart_item)
